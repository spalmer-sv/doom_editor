...
/*   Updated: 2021/01/16 18:28:38 by spalmer          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vertex	*new_vertex(int x, int y, t_all *all)
{
	...
	temp->x = x / all->setup.scale; 
	temp->y = y / all->setup.scale;
	temp->next = NULL;
	return (temp);
}
...
void	draw_vertex(t_vertex *vertex, t_all *all, int i)
{
	...
	while(temp)
	{
		...
		rect = init_rect((temp->x - (i / 2)) * all->setup.scale, (temp->y - (i / 2)) * all->setup.scale, i * all->setup.scale, i * all->setup.scale);
